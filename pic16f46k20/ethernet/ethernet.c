...
unsigned char   *arpResolv(unsigned char *ip)
{
        ...
        while(tmr < 45)         // allows around 3 seconds for reply
        {
                 ...
                 */
                rdptr = (Spi_Ethernet_readReg(ERDPTH) << 8) + Spi_Ethernet_readReg(ERDPT);
                ...
                && (Spi_Ethernet_memcmp(REPLY_START + 28, ip, 4) == 0))  // is it my IP addr ?
                {
                        ...
                        Spi_Ethernet_writeAddr(ERDPT, rdptr);
...
void sendUDP(unsigned char *destIP, unsigned int sourcePort, unsigned int destPort, unsigned char *pkt, unsigned int pktLen)
{
        ...
        Spi_Ethernet_writeMemory(REPLY_START + 24, Spi_Ethernet_readReg(EDMACSH), Spi_Ethernet_readReg(EDMACS));
        ...
        Spi_Ethernet_writeMemory(REPLY_START + 40, Spi_Ethernet_readReg(EDMACSH), Spi_Ethernet_readReg(EDMACS));
